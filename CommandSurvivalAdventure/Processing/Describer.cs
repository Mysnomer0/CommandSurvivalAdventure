...
namespace CommandSurvivalAdventure.Processing
{
    ...
    class Describer
    {
        ...
        // Returns a description of the given GameObject
        public static string GetUpdate(World.Chunk chunkToDescribe, World.GameObject objectRecievingDescription)
        {
            // The description to return
            string description = "\n\n";
            // The random generator
            ...
            #region Describe the biome
            // If the biome has changed then describe the biome with a little variation
            if (logChunk.biome.name != chunkToDescribe.biome.name)
            {
                if (random.Next(0, 1) == 0)
                {
                    description += "You leave the ";
                    description += logChunk.biome.name;
                    description += " and walk into ";
                }
                else
                {
                    description += "You exit the ";
                    description += logChunk.biome.name;
                    description += " into ";
                }
            ...
            {
                // Remind player of biome
                description += "You continue through the ";
                description += ToColor(chunkToDescribe.biome.name, chunkToDescribe.biome.associatedColor);
                description += ". ";
            }
            #endregion
            ...
            #region Describe the weather
            // If the wind has changed by over 20 then describe the wind speed
            if (Math.Abs(chunkToDescribe.windSpeed - logChunk.windSpeed) >= 20)
            {
                if (chunkToDescribe.windSpeed < 5)
                    description += "\nThe air here is still";
                else if (chunkToDescribe.windSpeed < 20)
                    description += "\nThe wind here is blowing gently";
                else if (chunkToDescribe.windSpeed < 40)
                    description += "\nThe wind in this area is $jablowing $jaharshly";
                else
                    description += "\nThe wind begins to $jahowl $jauncontrollably";
                // if the temp also changed tack the temp on the end of the wind
                if (Math.Abs(chunkToDescribe.temperature - logChunk.temperature) >= 20)
                {
                    if (chunkToDescribe.temperature < 20)
                        description += " and $lachills $layou $lato $lathe $labone. ";
                    else if (chunkToDescribe.temperature < 40)
                        description += "; making you $lacold. ";
                    else if (chunkToDescribe.temperature < 60)
                        description += " and mildly cold. ";
                    else if (chunkToDescribe.temperature < 80)
                        description += "; making you $gawarm. ";
                    else if (chunkToDescribe.temperature < 100)
                        description += "; making you $gahot. ";
                    else
                        description += " and the temperature is $eavery $eahot. ";
                }
                else
                {
                    description += ". ";
            ...
            else
            {
                if (Math.Abs(chunkToDescribe.temperature - logChunk.temperature) >= 20)
                {
                    // Describe the temperature
                    if (chunkToDescribe.temperature < 20)
                        description += "\nThe temperature becomes $lavery $lacold. ";
                    else if (chunkToDescribe.temperature < 40)
                        description += "\nThe temperature becomes $lacold. ";
                    else if (chunkToDescribe.temperature < 60)
                        description += "\nThe temperature becomes mildly cold. ";
                    else if (chunkToDescribe.temperature < 80)
                        description += "\nThe temperature becomes $gawarm. ";
                    else if (chunkToDescribe.temperature < 100)
                        description += "\nThe temperature becomes $gahot. ";
                    else
                        description += "\nThe temperature becomes $eavery $eahot. ";
                }
            }
            #endregion
            ...
            foreach (World.GameObject gameObject in firstOfEachGameObject)
            {
                ...
                if (amountOfEachObject[gameObject.type] == 1)
                {
                    subject = "There";
                    verbPhrase = "is";
                    prepositionalPhrase = "a " + ToColor(gameObject.identifier.name, gameObject.specialProperties.ContainsKey("colorAdd") ? gameObject.specialProperties["colorAdd"] : "") + " nearby";
                    // prepositionalPhrase = "a nearby " + ToColor(gameObject.identifier.fullName, gameObject.specialProperties.ContainsKey("colorAdd") ? gameObject.specialProperties["colorAdd"] : "");
                }
                // If there are relativly few, print accordingly
                else if (amountOfEachObject[gameObject.type] <= 10)
                {
                    subject = "There";
                    verbPhrase = "are";
                    prepositionalPhrase = "some " + ToColor(gameObject.identifier.name.Pluralize(), gameObject.specialProperties.ContainsKey("colorAdd") ? gameObject.specialProperties["colorAdd"] : "") + " nearby";
                    // prepositionalPhrase = "some nearby " + ToColor(gameObject.identifier.fullName.Pluralize(), gameObject.specialProperties.ContainsKey("colorAdd") ? gameObject.specialProperties["colorAdd"] : "");
                    /*
                    // If the word ends in "s", add an "es" to the end
                    if (gameObject.identifier.name[(gameObject.identifier.name.Length == 0) ? 0 : gameObject.identifier.name.Length - 1] == 's')
                ...
                else
                {
                    subject = "There";
                    verbPhrase = "are many";
                    prepositionalPhrase = ToColor(gameObject.identifier.name.Pluralize(), gameObject.specialProperties.ContainsKey("colorAdd") ? gameObject.specialProperties["colorAdd"] : "") + " nearby";
                    // prepositionalPhrase = ToColor(gameObject.identifier.fullName.Pluralize(), gameObject.specialProperties.ContainsKey("colorAdd") ? gameObject.specialProperties["colorAdd"] : "");
                    /*
                    // If the word ends in "s", don't change it to plural
                    if (gameObject.identifier.name[(gameObject.identifier.name.Length == 0) ? 0 : gameObject.identifier.name.Length - 1] == 's')
